...
inline void DataFrame::addCol(std::shared_ptr<Column<T>> col, ExtString name) {
    ...
    if (__schema.add_column(Schema::colToType(*col), *name))
        __data.push_back(std::make_shared<Column<T>>(col));
    else
...
inline T DataFrame::getVal(size_t col, size_t row) {
    ...
    auto dfcol = std::dynamic_pointer_cast<Column<T>>(__data.at(col));
